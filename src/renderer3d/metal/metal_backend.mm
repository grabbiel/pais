...
#include "pixel/renderer3d/metal/renderer.hpp"
...
std::unique_ptr<MetalShader> MetalShader::create(
    id<MTLDevice> device,
    ...
    shader->vertex_function_ = [shader->library_ newFunctionWithName:@"vertex_main"];
    shader->fragment_function_ = [shader->library_ newFunctionWithName:@"fragment_main"];
    
    if (!shader->vertex_function_ || !shader->fragment_function_) {
        // Try instanced shaders
        shader->vertex_function_ = [shader->library_ newFunctionWithName:@"vertex_instanced"];
        shader->fragment_function_ = [shader->library_ newFunctionWithName:@"fragment_instanced"];
    }
    ...
    shader->pipeline_state_ = [device newRenderPipelineStateWithDescriptor:pipelineDesc error:&error];
    ...
    shader->depth_stencil_state_ = [device newDepthStencilStateWithDescriptor:depthDesc];
...
std::unique_ptr<MetalMesh> MetalMesh::create(
    id<MTLDevice> device,
    ...
    mesh->vertex_buffer_ = [device newBufferWithBytes:vertices.data()
                                              length:vertices.size() * sizeof(Vertex)
                                             options:MTLResourceCPUCacheModeWriteCombined];
    
    // Create index buffer
    mesh->index_buffer_ = [device newBufferWithBytes:indices.data()
...
std::unique_ptr<MetalTexture> MetalTexture::create(
    id<MTLDevice> device,
    int width,
    int height,
    const uint8_t* data
...
std::unique_ptr<MetalTextureArray> MetalTextureArray::create(
    id<MTLDevice> device,
    int width,
    int height,
    int layers
) {
    auto textureArray = std::unique_ptr<MetalTextureArray>(new MetalTextureArray());
...
void MetalTextureArray::set_layer(int layer, const uint8_t* data) {
    if (layer < 0 || layer >= layers_ || !data) return;
...
void MetalBackend::begin_frame(const Color& clear_color) {
    ...
    // Set clear color
    render_pass_descriptor_.colorAttachments[0].clearColor = 
        MTLClearColorMake(clear_color.r, clear_color.g, clear_color.b, clear_color.a);
    
    // Create render encoder
    render_encoder_ = [command_buffer_ renderCommandEncoderWithDescriptor:render_pass_descriptor_];
...
void MetalBackend::setup_render_pass_descriptor() {
    ...
    // Color attachment
    render_pass_descriptor_.colorAttachments[0].texture = current_drawable_.texture;
...
std::unique_ptr<MetalShader> MetalBackend::create_shader(
    const std::string& vertex_src,
    const std::string& fragment_src
) {
...
std::unique_ptr<MetalMesh> MetalBackend::create_mesh(
    const std::vector<Vertex>& vertices,
    const std::vector<uint32_t>& indices
) {
...
std::unique_ptr<MetalTexture> MetalBackend::create_texture(
    int width,
    int height,
    const uint8_t* data
) {
...
std::unique_ptr<MetalTextureArray> MetalBackend::create_texture_array(
    int width,
    int height,
    int layers
) {
...
void MetalBackend::draw_mesh(
    ...
    if (!shader || !render_encoder_) return;
